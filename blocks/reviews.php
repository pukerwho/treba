<div class="reviews pt-5">
	<div class="container">
		<div class="row">
			<div class="col-md-5">
				<h2 class="treba-animate mb-5">Reviews</h2>
				<div class="reviews__text treba-animate">
					We do our best in every desicion we make throughout the day to make you happy with final design and implementation.
				</div>
				<div class="reviews__quote">
					<img src="<?php bloginfo('template_url') ?>/img/quote.svg" alt="">
				</div>
			</div>
			<div class="col-md-6 offset-md-1 pt-5">
				<div class="reviews__item treba-animate">
					<div class="reviews__item-avatar">
						<img src="<?php bloginfo('template_url') ?>/img/reviews1.png" alt="">
					</div>
					<div class="reviews__item-text mb-2">
						I want to say thanks to the guys from Treba Solutoins, especially to Kirill. Their work was done promptly, creatively, and most importantly, on time.
					</div>
					<div class="reviews__item-name">
						Katerina Golitsyna
					</div>
					<div class="reviews__item-pos">
						Famous singer and presenter 🎤
					</div>
				</div>
				<div class="reviews__item treba-animate">
					<div class="reviews__item-avatar">
						<img src="<?php bloginfo('template_url') ?>/img/reviews2.jpg" alt="">
					</div>
					<div class="reviews__item-text mb-2">
						The team from Treba Solutions immediately understood the idea of the site and our needs. This greatly simplified the work. It was easy and pleasant to work with them.
					</div>
					<div class="reviews__item-name">
						Vladimir Lenskiy
					</div>
					<div class="reviews__item-pos">
						Co-founder Vidpochivai
					</div>
				</div>
				<div class="reviews__item treba-animate">
					<div class="reviews__item-avatar">
						<img src="<?php bloginfo('template_url') ?>/img/reviews3.png" alt="">
					</div>
					<div class="reviews__item-text mb-2">
						We have been working (with these guys) for several months - I am completely satisfied with the result. The site is a blast!
					</div>
					<div class="reviews__item-name">
						Alexander
					</div>
					<div class="reviews__item-pos">
						Director WorldMoving
					</div>
				</div>
			</div>
		</div>
	</div>
</div>